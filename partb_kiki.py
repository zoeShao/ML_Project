...
def collect_user_question_meta():
    """
    Generate a dictionary which keys are question id and values are user id who
    ...
    with open(train_path, "r") as csv_file:
        ...
        for row in data:
            if row[0] != "question_id":
                if row[0] not in question_user_dict:
                    question_user_dict[row[0]] = [row[1]]
                else:
                    question_user_dict[row[0]].append(row[1])
    return question_user_dict
...
def knn_for_ques(question_id, question_dict, k, sparse_matrix):
    ...
    # The following print out is the experiment to test your hypothesis for Part B: Q3
    # Find the nearest k neighbours
    print("Current question id")
    print(question_id)
    correct_users = []
    question_user_dict = collect_user_question_meta()
    if str(question_id) in question_user_dict:
        users = question_user_dict[str(question_id)]
        for user in users:
            if sparse_matrix[int(user)][question_id] == 1:
                correct_users.append(user)
        print("The students who correctly answer current question")
        print(correct_users)

    neighbors = []
    for x in range(k):
        neighbors.append(distances[x][0])
    print("K nearest neighbours question id")
    print(neighbors)
    print("The subjects of these K nearest neighbours questions")
    neighbors_subject = []
    user_neighbor_acc = []
    for neighbor_id in neighbors:
        neighbors_subject.append(question_dict[neighbor_id][1])
        if correct_users != []:
            acc = 0
            total_person = 0
            for user in correct_users:
                if sparse_matrix[int(user)][neighbor_id] in (0, 1):
                    total_person += 1
                    if sparse_matrix[int(user)][neighbor_id] == 1:
                        acc += 1
            if total_person != 0:
                user_neighbor_acc.append(acc/total_person)
            else:
                user_neighbor_acc.append(None)
    print(neighbors_subject)
    print("The accuracy of each K nearest neighbours questions answered by those students who correctly answer current question")
    print(user_neighbor_acc)
...
def predict_all_ques(sparse_matrix, question_dict, k):
    ...
    for q_id in range(question_nums):
        question_pred = knn_for_ques(q_id, question_dict, k, sparse_matrix)
